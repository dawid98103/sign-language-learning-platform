import React, { useState, useEffect } from 'react';
import MarginContainer from '../component/MarginContainer';
import AxiosClient from '../config/axios/AxiosClient';
import LearnContainer from '../component/LearnContainer';
import GlobalSpinner from '../component/GlobalSpinner';

function LearnPage({ match }) {

    const [currentElement, setCurrentElement] = useState(0)
    const [elements, setElements] = useState([]);

    useEffect(() => {
        fetchElements();
    }, []);

    const fetchElements = async () => {
        console.log("blbla");
        const { data } = await AxiosClient.get(`/lessons/${match.params.lessonId}/stage/${match.params.stageId}/element`)
        setElements(data);
        console.log(data);
    }

    const nextElement = () => {
        if (elements.length > currentElement + 1) {
            setCurrentElement(currentElement + 1)
        }
    }

    const prevElement = () => {
        if (currentElement - 1 >= 0) {
            setCurrentElement(currentElement - 1)
        }
    }

    return (
        <MarginContainer>
            {elements.length > 0
                ? <LearnContainer videoUrl={elements[currentElement].videoUrl} description={elements[currentElement].description} currentElement={currentElement + 1} quantityElements={elements.length} nextElement={() => nextElement()} prevElement={() => prevElement()} />
                : <GlobalSpinner />}
        </MarginContainer>
    )
}

export default LearnPage

